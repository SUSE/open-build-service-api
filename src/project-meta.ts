import { BaseProjectMeta, BaseRepository } from "./api/base-types";
import {
  RepositorySetting,
  repositorySettingFromFlag,
  repositorySettingToFlag
} from "./api/flag";
import * as api from "./api/project-meta";
...
function convertFromApiProject(apiProject: api.ProjectMeta): ProjectMeta {
  const {
    build,
    publish,
    useForBuild,
...
function convertToApiProject(proj: ProjectMeta): api.ProjectMeta {
...
export async function getProjectMeta(
  ...
  const proj = await api.getProjectMeta(con, name);
